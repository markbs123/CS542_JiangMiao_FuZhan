...
public class Connection {
	...
	/**
	 * Distribution the opdata which contains key and value into two array lists
	...
	public Connection(Connection opdata) {
		this.key = Arrays.copyOf(opdata.getallkeys(),
				opdata.getallkeys().length);
		this.value = Arrays.copyOf(opdata.getallvalues(),
				opdata.getallvalues().length);
	}

	/**
	 * Definition the Select method to get required data from the relation,
	 * remove other useless data
	 * 
	 * @param names
	 *            required attributes
	 */
	...
	/**
	 * Translate all the connection to the String value, ensure the final
	 * results are in data form
	 */
